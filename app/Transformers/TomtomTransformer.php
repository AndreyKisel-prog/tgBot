...
class TomtomTransformer extends BaseTransformer
{
    ...
    public function handle(): array
    {
        ...
        $results = [];
        if ($countResults) {
            foreach ($rawResults as $index => $item) {
                $place = $item["poi"] ?? null;
    ...
    public function createMessages($data)
    {
        ...
        foreach ($data['results'] as $index => $item) {
            ...
            if (isset($item['name'])) $firmDescription[] = "Название: " . $item['name'] . PHP_EOL;
            if (isset($item['score'])) $firmDescription[] = "Оценка заведения: " . $item['score'] . PHP_EOL;
            if (isset($item['dist'])) $firmDescription[] = "Расстояние до: " . $item['dist'] . PHP_EOL;
            if (isset($item['phone'])) $firmDescription[] = "Тел: " . $item['phone'] . PHP_EOL;
            if (isset($item['address'])) $firmDescription[] = "Адрес: " . $item['address'] . PHP_EOL;
            if (isset($item['url'])) $firmDescription[] = "Сайт: " . $item['url'] . PHP_EOL;
